  ...
  <div class="p-4">
    ...
    <template v-else-if="task">
      ...
      <div v-if="task.result" class="rounded-lg border p-4 mb-4">
        ...
        <pre v-if="task.result.stdout" class="mt-2 text-xs bg-muted rounded p-2 overflow-x-auto max-h-48 overflow-y-auto whitespace-pre-wrap break-words">{{ task.result.stdout }}</pre>
        <pre v-if="task.result.stderr" class="mt-2 text-xs bg-severity-critical-muted text-severity-critical-foreground rounded p-2 overflow-x-auto max-h-32 overflow-y-auto whitespace-pre-wrap break-words">{{ task.result.stderr }}</pre>
      </div>
