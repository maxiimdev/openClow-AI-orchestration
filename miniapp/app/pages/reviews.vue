...
import { Wrench, AlertOctagon, GitBranch } from 'lucide-vue-next'
...
<template>
  <div class="p-4 sm:p-6">
    <div class="mb-5">
      <h1 class="text-2xl font-bold tracking-tight">Review Center</h1>
    ...
    <template v-else>
      <!-- Summary bar -->
      <div class="flex items-center gap-3 mt-4 mb-5 p-3 rounded-xl bg-muted/40 border border-border">
        <Badge variant="secondary" class="font-semibold tabular-nums">{{ summary.total }} total</Badge>
        <Separator orientation="vertical" class="h-4" />
        <Badge v-if="summary.passed" class="bg-success-muted text-success-muted-foreground border-0 tabular-nums">{{ summary.passed }} passed</Badge>
        <Badge v-if="summary.failed" class="bg-severity-major-muted text-severity-major-foreground border-0 tabular-nums">{{ summary.failed }} failed</Badge>
        <Badge v-if="summary.escalated" class="bg-severity-critical-muted text-severity-critical-foreground border-0 tabular-nums">{{ summary.escalated }} escalated</Badge>
      </div>
        ...
        >
          <Card class="card-interactive">
            <CardContent class="pt-4 pb-4">
              ...
              <p class="mt-2 text-sm text-muted-foreground leading-relaxed line-clamp-2">{{ getReviewCardSummary(task) }}</p>

              <!-- Severity breakdown -->
              <div v-if="findingsByTask[task.id]" class="mt-2.5 flex gap-1.5">
                <template v-for="(count, sev) in findingsByTask[task.id]" :key="sev">
                  <Badge
                    v-if="count > 0"
                    variant="secondary"
                    :class="[severityClass[sev], 'border-transparent text-xs tabular-nums']"
                  >
                    {{ count }} {{ sev }}
                  </Badge>
                </template>
              ...
              <div v-if="task.status === 'review_fail' && canRequestPatch(task)" class="mt-2.5 flex items-center gap-1.5 text-xs text-severity-major-foreground">
                ...
                <span class="font-medium">Patch available</span>
              ...
              <div v-else-if="task.status === 'escalated'" class="mt-2.5 flex items-center gap-1.5 text-xs text-severity-critical-foreground">
                ...
                <span class="font-medium">Manual intervention required</span>
              </div>

              <div class="mt-2.5 text-xs text-muted-foreground/70">
