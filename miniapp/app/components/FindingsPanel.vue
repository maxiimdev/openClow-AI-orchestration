    ...
    <Card v-for="f in findings" :key="f.id">
      <CardContent class="pt-4 pb-4">
        <div class="flex items-center gap-2">
          <Badge variant="secondary" :class="severityColor[f.severity] || severityColor.major" class="text-xs uppercase tracking-wide">
            {{ f.severity }}
          </Badge>
        ...
        <p class="mt-2.5 text-sm font-medium leading-snug">{{ f.issue }}</p>
        <div class="mt-2.5 space-y-1.5 text-[0.8125rem]">
          <p class="text-muted-foreground"><span class="font-semibold text-foreground/80">Risk:</span> {{ f.risk }}</p>
          <p class="text-muted-foreground"><span class="font-semibold text-foreground/80">Fix:</span> {{ f.required_fix }}</p>
        </div>
      </CardContent>
    </Card>
  </div>
