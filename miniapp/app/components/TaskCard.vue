    ...
    <Card class="card-interactive">
      <CardContent class="pt-4 pb-4">
        <div class="flex items-center justify-between gap-2">
          <span class="font-mono text-[0.8125rem] text-muted-foreground truncate">{{ truncateId(task.id) }}</span>
        ...
        <div class="mt-2.5 text-xs text-muted-foreground/70">{{ formatRelativeTime(task.updatedAt) }}</div>
      </CardContent>
    </Card>
  </NuxtLink>
</template>
