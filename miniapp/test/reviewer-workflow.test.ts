...
describe('canRequestPatch', () => {
  it('returns true for review_fail with iterations remaining', () => {
    const task = makeTask({ status: 'review_fail', meta: { reviewIteration: 1, reviewMaxIterations: 3 } })
    expect(canRequestPatch(task)).toBe(true)
  })
  it('returns false for review_fail at max iterations', () => {
    const task = makeTask({ status: 'review_fail', meta: { reviewIteration: 3, reviewMaxIterations: 3 } })
  ...
  it('defaults maxIterations to 3 when not set', () => {
    const task = makeTask({ status: 'review_fail', meta: { reviewIteration: 2 } })
    expect(canRequestPatch(task)).toBe(true)
  })
  it('returns true when meta has no iteration info', () => {
...
describe('buildReviewDiffSummary', () => {
  it('identifies resolved, remaining, and new findings', () => {
    ...
    const curr = [makeFinding({ id: 'F2' }), makeFinding({ id: 'F3' })]
...
describe('reviewer status transition scenarios', () => {
  const scenarios: Array<{ name: string; from: Task['status']; canPatch: boolean; canReReview: boolean; meta?: Record<string, unknown> }> = [
    { name: 'review_fail iter 1/3', from: 'review_fail', canPatch: true, canReReview: true, meta: { reviewIteration: 1, reviewMaxIterations: 3 } },
    { name: 'review_fail iter 2/3', from: 'review_fail', canPatch: true, canReReview: true, meta: { reviewIteration: 2, reviewMaxIterations: 3 } },
    { name: 'review_fail iter 3/3', from: 'review_fail', canPatch: false, canReReview: true, meta: { reviewIteration: 3, reviewMaxIterations: 3 } },
    { name: 'escalated', from: 'escalated', canPatch: false, canReReview: true },
    { name: 'review_pass', from: 'review_pass', canPatch: false, canReReview: false },
    { name: 'running', from: 'running', canPatch: false, canReReview: false },
    { name: 'completed', from: 'completed', canPatch: false, canReReview: false },
    { name: 'needs_input', from: 'needs_input', canPatch: false, canReReview: false },
    { name: 'failed', from: 'failed', canPatch: false, canReReview: false },
  ]
...
const ReviewDetailPage = defineComponent({
  ...
  setup(props) {
    ...
    return { reReviewLoading, reReviewError, iterationInfo, canRequestPatch, canRequestReReview, getReviewCardSummary, truncateId, formatRelativeTime, countFindingsBySeverity }
  },
  template: `
    <div>
      ...
      <div v-if="task.status === 'review_fail'" data-testid="review-fail-banner">
        ...
        <button v-if="canRequestPatch(task)" data-testid="btn-patch-rereview" :disabled="reReviewLoading">Request Patch</button>
        <button v-if="canRequestReReview(task)" data-testid="btn-rereview-only" :disabled="reReviewLoading">Re-review Only</button>
      ...
      <div v-else-if="task.status === 'escalated'" data-testid="escalated-banner">
        <span>Escalated</span>
        <button v-if="canRequestReReview(task)" data-testid="btn-force-rereview" :disabled="reReviewLoading">Force Re-review</button>
      </div>
...
describe('Review detail page rendering', () => {
  it('shows review_fail banner with patch and re-review buttons', () => {
    const task = makeTask({ status: 'review_fail', meta: { reviewIteration: 1, reviewMaxIterations: 3 }, structuredFindings: [makeFinding()] })
  ...
  it('shows escalated banner with force re-review button', () => {
    const task = makeTask({ status: 'escalated', structuredFindings: [makeFinding({ severity: 'critical' })] })
    const wrapper = mount(ReviewDetailPage, { props: { task } })
  ...
  it('shows no review banners for review_pass', () => {
    const wrapper = mount(ReviewDetailPage, { props: { task: makeTask({ status: 'review_pass' }) } })
  ...
  it('shows no review banners for running', () => {
    const wrapper = mount(ReviewDetailPage, { props: { task: makeTask({ status: 'running' }) } })
    expect(wrapper.find('[data-testid="review-fail-banner"]').exists()).toBe(false)
    expect(wrapper.find('[data-testid="escalated-banner"]').exists()).toBe(false)
  })
  it('renders structured findings', () => {
    const task = makeTask({
      status: 'review_fail',
      structuredFindings: [makeFinding({ id: 'F1', severity: 'critical', issue: 'XSS bug' }), makeFinding({ id: 'F2', severity: 'minor', issue: 'Style issue' })],
    })
    const wrapper = mount(ReviewDetailPage, { props: { task } })
  ...
  it('renders review iteration history when multiple events', () => {
  ...
  it('hides iteration history when only one event', () => {
    const reviewEvents: TaskEvent[] = [
      { id: 'e1', taskId: 'task-001', status: 'review_fail', phase: 'review', message: 'Fail', meta: {}, createdAt: '2025-01-01T00:00:00Z' },
    ]
    const wrapper = mount(ReviewDetailPage, { props: { task: makeTask({ status: 'review_fail' }), reviewEvents } })
    expect(wrapper.find('[data-testid="iteration-history"]').exists()).toBe(false)
  })
})

const ReviewsListPage = defineComponent({
  props: { tasks: { type: Array as () => Task[], default: () => [] } },
  setup(props) {
    const reviewTasks = computed(() => filterReviewTasks(props.tasks))
    function iterationsLeft(task: Task) { const info = getIterationInfo(task); return info ? info.remaining : null }
    return { reviewTasks, getReviewCardSummary, canRequestPatch, iterationsLeft }
  },
  template: `
    <div>
      <div v-for="task in reviewTasks" :key="task.id" :data-testid="'card-' + task.id" class="card">
        ...
        <div v-if="task.status === 'review_fail' && canRequestPatch(task)" data-testid="action-patch" class="action-hint">
          Patch available
          <span v-if="iterationsLeft(task) !== null" class="iterations-left">{{ iterationsLeft(task) }} iterations left</span>
        </div>
        <div v-else-if="task.status === 'escalated'" data-testid="action-escalated" class="action-hint">Manual intervention required</div>
      </div>
    </div>
  `,
})
...
describe('Reviews list page action hints', () => {
  it('shows patch hint for review_fail with iterations remaining', () => {
    const wrapper = mount(ReviewsListPage, { props: { tasks: [makeTask({ id: 'rf', status: 'review_fail', meta: { reviewIteration: 1, reviewMaxIterations: 3 } })] } })
  ...
  it('hides patch hint at max iterations', () => {
    const wrapper = mount(ReviewsListPage, { props: { tasks: [makeTask({ id: 'rf', status: 'review_fail', meta: { reviewIteration: 3, reviewMaxIterations: 3 } })] } })
    expect(wrapper.find('[data-testid="action-patch"]').exists()).toBe(false)
  })
  it('shows escalated hint', () => {
    const wrapper = mount(ReviewsListPage, { props: { tasks: [makeTask({ id: 'esc', status: 'escalated' })] } })
    expect(wrapper.find('[data-testid="action-escalated"]').exists()).toBe(true)
  })
  it('no action hints for review_pass', () => {
    const wrapper = mount(ReviewsListPage, { props: { tasks: [makeTask({ id: 'rp', status: 'review_pass' })] } })
    expect(wrapper.find('[data-testid="action-patch"]').exists()).toBe(false)
